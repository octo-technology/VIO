...
async def aget_binaries_annotations_path(
    url_orchestrator: str, config_name: str
) -> dict:
    dict_metabin = {}
        ...
        for camera_id in item_metadata["inferences"].keys():
            ...
            for model_id in camera_detections.keys():
                for detection in camera_detections[model_id].values():
                    lb_annotation = transform_annotations_to_bbox(
                        detection, camera_dimensions
                    )
                    lb_annotation_list.append(lb_annotation)

            # Get binary path
            call_url = (
                f"{url_orchestrator}/api/v1/binary_path/"
                f"{item_id}/{camera_id}/{config_name}"
            )
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(call_url) as response:
...
def transform_annotations_to_bbox(
    annotation: dict, camera_dimensions: List[int]
    ...
    bbox_annotation = lb_types.ObjectAnnotation(
        ...
        value=lb_types.Rectangle(
            start=lb_types.Point(x=location[0] * img_width, y=location[1] * img_height),
            # x = left, y = top
            end=lb_types.Point(
                x=location[2] * img_width, y=location[3] * img_height
...
class LabelboxLabelizer(Labelizer):
    ...
    async def apost_images(
        ...
        try:
            ...
            logger.warning(
                f"Error while uploading labelbox MAL predictions: Error: {err}"
            )
            return False
