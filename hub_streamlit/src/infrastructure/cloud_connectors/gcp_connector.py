...
from src.infrastructure.cloud_connectors.edge_data import Camera, Edge, EdgeData, Item, UseCase
...
def extract_items(_gcp_client: Client) -> EdgeData:
    ...
    for blob in blobs_images_sorted:
        ...
        if camera_id not in edges_data.edges[edge_name].use_cases[use_case].items[item_id].camera_names:
            edges_data.edges[edge_name].use_cases[use_case].items[item_id].camera_names.append(camera_id)
            edges_data.edges[edge_name].use_cases[use_case].items[item_id].cameras[camera_id] = Camera()

        if ".jpg" in file_name:
            # Downloading the first 5 pics
            if edges_data.edges[edge_name].use_cases[use_case].items[item_id].number_pictures < 5:
                binary_data = blob.download_as_bytes()
                img = Image.open(BytesIO(binary_data))

                # If metadata is not empty, we plot the predictions
                if filtering_items_that_have_predictions(edges_data.edges[edge_name].use_cases[use_case].items[item_id].metadata, camera_id):
                    img = plot_predictions(img, camera_id,
                                           metadata=edges_data.edges[edge_name].use_cases[use_case].items[item_id].metadata)
            else:
                img = Image.new("RGB", (100, 100), (200, 155, 255))

            edges_data.edges[edge_name].use_cases[use_case].items[item_id].cameras[camera_id].pictures.append(img)
            edges_data.edges[edge_name].use_cases[use_case].items[item_id].number_pictures += 1
