...
from supervisor.api_config import get_metadata_storage, get_binary_storage, get_model_forward, get_edge_station, \
...
from supervisor.domain.models.item import get_item_rule
...
class Supervisor:
    ...
    def __init__(self, metadata_storage=get_metadata_storage(), binary_storage=get_binary_storage(),
                 model_forward=get_model_forward(), station_config=get_station_config(),
                 edge_station=get_edge_station(), telemetry_sink=get_telemetry_sink()):
    ...
    async def get_prediction_for_camera(self, camera_id: str, item: Item,
                                        image_name: str) -> Dict[str, Union[Dict, Any]]:
        inference_output = {}
    ...
    async def get_inference(self, inference_output: Dict, model_pipeline: List[ModelInfos],
                            full_image: bytes, image_name: str) -> Dict[str, Dict]:
        for current_model in model_pipeline:
            ...
            if _model_has_no_dependency(current_model.depends_on):
                inference_output[current_model.id] = await self.model_forward.perform_inference(current_model,
                                                                                                full_image,
                                                                                                image_name)
            else:
                inference_output[current_model.id] = {}
                model_dependencies = [model_infos for model_infos in model_pipeline
                                      if model_infos.id in current_model.depends_on]
                inference_output_dependencies = await self.get_inference(inference_output, model_dependencies,
                                                                         full_image, image_name)
                for model_dependency in model_dependencies:
                    for object_id, inference_output_dependency in inference_output_dependencies[
                        model_dependency.id].items():  # noqa
                        object_location = inference_output_dependency['location']
                        cropped_image = crop_image(full_image, object_location)
                        inference_output_object = await self.model_forward.perform_inference(current_model,
                                                                                             cropped_image,
                                                                                             object_id)
                        for sub_object_id, sub_object_info in inference_output_object.items():
...
def relocate_sub_object_location_within_full_image(object_location: List[int], sub_object_location: List[int]) -> \
        List[int]:
