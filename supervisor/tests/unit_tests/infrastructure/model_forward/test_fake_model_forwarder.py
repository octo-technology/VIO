...
from supervisor.domain.ports.model_forward import Labels
...
class TestFakeModelForwarder:
    async def test_perform_inference_should_return_classification_results(self):
        ...
        model_inference_version = ModelInfos(id='model1', depends_on=[], name='inception',
                                             category='classification',
                                             version='1.2', camera_id='camera_id1', image_resolution=[640, 640])
        binary_data = bytes([])

        expected = {
            'full_image': {
                'label': Labels.OK.value,
                'probability': 0.3745401188473625
            }
        }
    ...
    async def test_perform_inference_should_return_object_detection_results(self):
        ...
        random.seed(42)
        fake_model_forwarder = FakeModelForward()
        model_inference_version = ModelInfos(id='model1', depends_on=[], name='mobilenet_v1_640x640',
                                             category='object_detection',
                                             version='1', camera_id='camera_id1', image_resolution=[640, 640])
        binary_data = bytes([])
    ...
    async def test_perform_inference_should_return_object_detection_with_classification_results(self):
        # Given
        np.random.seed(42)
        random.seed(42)
        fake_model_forwarder = FakeModelForward()
        model_inference_version = ModelInfos(id='model1', depends_on=[], name='mobilenet_v1_640x640_detect_classif',
                                             category='object_detection_with_classification',
                                             version='1.3', camera_id='camera_id1')
        binary_data = bytes([])
