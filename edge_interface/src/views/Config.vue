<template>
  <div class="container">
    ...
    <v-card
        v-for="(item_config, item_category) in configs" :key="item_category" elevation="5"
        class="config-card"
    >
          ...
          <tr>
            <td v-for="(camera_config, camera_id) in item_config['cameras']" :key="camera_id">
              ...
              <div v-for="(model, model_id) in camera_config['models_graph']" :key="model_id">
                ...
                <div> Model type: {{inventory["models"][model["metadata"]]["category"]}} </div>
                <div> Model version: {{inventory["models"][model["metadata"]]["version"]}} </div>
                <div> Class names: {{inventory["models"][model["metadata"]]["class_names"]}} </div>
             ...
             <div v-for="(camera_rule, camera_rule_id) in camera_config['camera_rule']['parameters']" :key="camera_rule_id" >
