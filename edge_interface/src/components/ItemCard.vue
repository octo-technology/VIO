<template>
  <v-card tile>
    <v-list-item>
      <v-list-item-content>
        <v-list-item-title class="headline mb-1">{{ serial_number }}</v-list-item-title>
      </v-list-item-content>
...
export default {
  ...
  computed: {
    has_category() {
      return (
        this.item != null && this.item != undefined && this.item.category != undefined && this.item.category != null
      )
    },
    has_received_time() {
      return (
