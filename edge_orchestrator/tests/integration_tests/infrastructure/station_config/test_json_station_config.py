...
from tests.conftest import TEST_INVENTORY_PATH, TEST_STATION_CONFIGS_FOLDER_PATH, TEST_DATA_FOLDER_PATH


@freeze_time(lambda: dt.datetime(year=2021, month=5, day=19, hour=15, minute=0, second=0))
class TestJsonStationConfig:

    def test_get_models_for_camera_should_return_one_model_infos_when_camera_config_has_one_model(self):
        # Given
    ...
    def test_get_models_for_camera_should_return_two_model_infos_when_camera_config_has_two_models(self):
        # Given
        inventory = JsonInventory(TEST_INVENTORY_PATH)
        json_station_config = JsonStationConfig(TEST_STATION_CONFIGS_FOLDER_PATH, inventory, TEST_DATA_FOLDER_PATH)
