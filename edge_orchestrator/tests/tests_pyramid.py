...
def measure_tests_pyramid_and_create_gitlab_badges(badges_generation: bool):
    ...
    print(f'''\n
        ...
        \033[92m  /_|__|__|\\/\`/\`.
         /|__|___|__\\/\`/
        /__|___|___|_\\/      [*] unit tests: {number_of_unit_tests}/{total_number_of_tests} --> {percentage_of_unit_tests:.2f} %\033[0m
\n''')  # noqa
    ...
    if badges_generation:
        ...
        anybadge.Badge(label='unit-tests-ratio', value=f'{percentage_of_unit_tests:.2f} %',
                       default_color=unit_color).write_badge(unit_test_file, overwrite=True)
        anybadge.Badge(label='integration-tests-ratio', value=f'{percentage_of_int_tests:.2f} %',
                       default_color=int_color).write_badge(int_test_file, overwrite=True)
        anybadge.Badge(label='functional-tests-ratio', value=f'{percentage_of_func_tests:.2f} %',
                       default_color=func_color).write_badge(func_test_file, overwrite=True)
...
def compute_number_of_each_test_type(
        ...
        test_scenario_pattern: re.Pattern = re.compile('Scenario:')) -> Tuple[int, int, int]:
...
def check_if_pyramid_is_ok(number_of_unit_tests: int, number_of_int_tests: int, number_of_func_tests: int):
    if number_of_func_tests > number_of_int_tests or \
            number_of_func_tests > number_of_unit_tests or \
            number_of_int_tests > number_of_unit_tests:
