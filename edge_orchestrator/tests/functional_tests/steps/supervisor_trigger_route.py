...
from tests.functional_tests.steps.common_steps import (
    assert_decision_is_valid,
    assert_metadata_almost_equal,
...
def config_is_activated(context: Context, config_name: str):
    ...
    context.execute_steps(
        """
            ...
            \"\"\"
        """.format(
            station_config_TEST=config_name, config=json.dumps(config)
        )
    )
...
def check_inference_is_computed(context: Context):
    ...
    context.execute_steps(
        """
            ...
            \"\"\"
        """.format(
            item_metadata=json.dumps(expected_item_metadata)
        )
    )
