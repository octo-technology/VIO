...
class MinNbObjectsRule(ICameraRule):
    def __init__(self, camera_rule_config: CameraRuleConfig):
        ...
        self.min_threshold: int = self._camera_rule_config.params["min_threshold"]
    ...
    def _get_camera_decision(self, prediction: Prediction) -> Decision:
        ...
        if detec_predict_with_classif.prediction_type != ModelType.OBJECT_DETECTION_WITH_CLASSIFICATION:
            self._logger.warning(
                f"You can not use an MinNbObjectsRule on something other than {ModelType.OBJECT_DETECTION_WITH_CLASSIFICATION.value}, no decision returned."
            )
