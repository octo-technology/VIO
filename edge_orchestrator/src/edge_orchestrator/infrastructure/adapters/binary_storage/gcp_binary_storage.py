...
class GCPBinaryStorage(IBinaryStorage):
    def __init__(self, station_config: StationConfig, storing_path_manager: StoringPathManager):
        ...
        self._bucket = self._storage_client.get_bucket(station_config.binary_storage_config.bucket_name.as_posix())
    ...
    def save_item_binaries(self, item: Item):
        for camera_id, image in item.binaries.items():
            blob = self._bucket.blob((self._storing_path_manager.get_storing_path(item.id) / f"{camera_id}.jpg").as_posix())
            if blob is None:
